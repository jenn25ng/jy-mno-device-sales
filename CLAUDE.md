...
## 6. 본부 8개 (판매 본부 — 표시 순서 고정, MAMF 리포트 기준)

수도권 · 부산 · 대구 · 서부 · 중부 · 유통 · 제휴 · 기업사업본부
- ⚠️ **리포트 기준 8개**. `유통` = 리포트 라벨 `PS&M`, 마트 org명은 `유통사업부`(2026-05 53,837). 8개 합 = **358,028**(2026-05) = 리포트 8본부 총실적.
- **`MNO AI마케팅`(25,771)·`air서비스본부`(4,253)는 리포트에서 본부 제외(체크 해제)라 미포함.** `Connectivity사업`·`Product&Brand본부`(각 소량)는 비판매라 제외. (전체 조직 무필터=388,058이지만 대시보드는 8본부 스코프)
