-- ═══════════════════════════════════════════════════════════════════════════
-- device_sales_summary_daily2  테이블 생성 (최초 1회)  — 파티션 Iceberg
-- ---------------------------------------------------------------------------
...
-- (교체 시) DROP TABLE obt_encore_max.device_sales_summary_daily2;

CREATE TABLE obt_encore_max.device_sales_summary_daily2 (
...
LOCATION 's3://your-bucket/obt_encore_max/device_sales_summary_daily2/'   -- ⚠️ 실제 버킷으로 교체
TBLPROPERTIES (
  'table_type' = 'ICEBERG',
  'format'     = 'parquet'
);
...
-- 주1회 스냅샷 정리 (별도 스케줄)
-- VACUUM obt_encore_max.device_sales_summary_daily2;
